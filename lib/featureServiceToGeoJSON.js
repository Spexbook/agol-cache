...
const wrapFetch = (fetch) => {
    ...
    return require('fetch-retry')(fetch, {
        retryOn: function (attempt, error, response) {
            if (attempt < 10 && error) {
                ...
                logger.warn(
                    `Retry attempt ${attempt + 1} ${JSON.stringify(error)}`
                );
                spinner.start();
                return true;
            }
...
const defaultFetch = wrapFetch(require('node-fetch'));
...
async function featureServiceToGeoJSON(featureServiceUrl, options, callback) {
    ...
    const featureServer =
        !featureServiceUrl.includes('MapServer') &&
        featureServiceUrl.includes('FeatureServer')
            ? true
            : false;
    const mapServer =
        featureServiceUrl.includes('MapServer') &&
    ...
    if (!mapServer && !featureServer) {
        throw new Error(
            'Error, this script only works with FeatureServer and MapServer services'
        );
    }
    ...
    const config = {
        ...
        esriIdField:
            !options || !options.esriIdField ? false : options.esriIdField, //field to use for the esriIdField, used in the query parameters
        ...
        silent:
            options && options.silent
                ...
                : options && !options.silent
                ? false
                : true, //turn off viewing log messages in the console, disabled if debug is set to false, however spinner is always on
        token: options.token ?? (env && env.TOKEN ? env.TOKEN : null), //token to use for secured routes,
        ...
        format:
            options &&
            options.format &&
            ['json', 'geojson'].includes(options.format.toLowerCase())
                ? options.format.toLowerCase()
                : 'geojson',
        steps: options.steps || 1000,
        fetch: wrapFetch(options.fetch ?? require('node-fetch')),
    };
    ...
    logger.transports.forEach((transport) => {
        if (transport.name && transport.name === 'console')
            transport.silent = config.silent;
        if (transport.name && transport.name === 'logfile')
            transport.silent = !config.debug;
    });
    ...
    console.log('Forked version of Agol-cache');
    const allLayers = getLayers(service);
    const layers = allLayers.filter(
        (l) => !config.filter || l.name.includes(config.filter)
    );
    ...
    for (let i = 0; i < layers.length; i++) {
        const l = layers[i];
        const domainFields = [];
        const layerDefinition = await config
            .fetch(
                `${featureServiceUrl}${l.id}/?f=json${
    ...
    const asyncHandleLayers = async () => {
        ...
        if (!total) {
            ...
            throw new Error('Fatal error,  no layers found.');
        }

        if (!config.silent)
            spinner.succeed('Layers found: ' + layers.length).start();
        ...
        if (options.layerByLayer) {
            for (let i = 0; i < layers.length; i++) {
                ...
                if (!config.silent)
                    spinner.info(l?.name + ' total features: ' + count);
                ...
                if (config.filter) {
                    if (!l.name.includes(config.filter)) {
                        if (!config.silent)
                            spinner.info('Skipping layer: ' + l.name);
                        total = total - 1;
                        if (!total) {
                            spinner.stop();
                            if (callback) {
                                ...
                                spinner.succeed(
                                    'Completed! No callback function found.'
                                );
                            }
                        }
                        continue;
...
async function getFeatureService(serviceURL, opts) {
    const url =
        opts && opts.token
            ...
            : serviceURL + '?f=json';
    try {
        const res = await (opts.fetch ?? defaultFetch)(url, {
...
function getLayers(serviceDefinition) {
    ...
    if (!serviceDefinition.layers || !serviceDefinition.layers.length)
        return layers;
    ...
    serviceDefinition.layers.forEach((l) => {
        ...
        if (!l.subLayerIds) {
            layers.push({
                ...
                renderer:
                    l.drawingInfo && l.drawingInfo.renderer
                        ? l.drawingInfo.renderer
                        : null,
            });
        }
    });
...
async function getFeatures(
    ...
    const filename = opts.fileName ?? `${opts.prefix}${layerName}`;
    ...
    if (!opts.esriIdField || opts.hasAttachments) {
        if (!opts.esriIdField && !opts.silent)
            spinner
                .info(
                    'Attempting to determing missing esriIdField for ' +
                        layerName
                )
                .start();
        ...
        try {
            ...
            const res = await (opts.fetch ?? defaultFetch)(
                `${layerUrl}?f=pjson${
                    opts.token ? `&token=${opts.token}` : ''
                }`,
                {
            ...
            if (!queryRes || !queryRes.fields || !queryRes.fields.length) {
                ...
                if (opts.debug)
                    logger.error('No fields found when querying layer.');
            ...
            if (!queryRes.hasAttachments) {
                ...
                if (opts.attachments && !getAttachments) {
                    spinner
                        .warn(
                            'Attachments requested but the layer does not have attachments.'
                        )
                        .start();
                    if (opts.debug)
                        logger.error(
            ...
            if (!idFieldObject.length) {
                spinner.warn(
                    'No esriFieldTypeOID field found for ' + layerName
                );
                if (opts.debug)
                    logger.error(
                        'No esriFieldTypeOID field found ' + layerName
                    );
                spinner.start('Processing');
            } else {
                if (!opts.silent)
                    spinner
                        .succeed(
                            'Found esriIdField for ' +
                                layerName +
                                ': ' +
                                idFieldObject[0].name
                        )
                        .start();
            }
        } catch (error) {
    ...
    const idField = !opts.esriIdField
        ? idFieldObject[0].name.toLowerCase()
        : opts.esriIdField;
        ...
        try {
            if (!opts.silent) {
                if (spinner.isSpinning) {
                    spinner.text =
                        'Querying features ' + ids.start + '-' + ids.end;
                } else {
                    spinner.start(
                        'Querying features ' + ids.start + '-' + ids.end
                    );
                    ...
                    geojson.features.forEach((f) => {
                        ...
                        if (domainFields && domainFields.length) {
                            domainFields.forEach((d) => {
                                ...
                                if (value) {
                                    const codedValue = values.filter(
                                        (v) => v.code === value
                                    );
            ...
            if (opts.debug)
                logger.error('Error in queryAGOL ' + error.toString());
            hasErrors = true;
            completed = true;
            // throw new Error(error)
        }
    ...
    if (hasErrors) {
        spinner
            .warn(
                `Warning! ${nbrFeatures} features found, there were errors with ${filename}`
            )
            .start();
        if (opts.debug)
            logger.error(
        ...
        spinner
            .succeed(`Success! Wrote ${nbrFeatures} features to ${filepath}`)
            .start();
        if (opts.debug)
            logger.debug(
                `Success! Wrote ${nbrFeatures} features to ${filepath}`
            );
    }
...
async function queryAGOL(url, idField, start, end, opts, layerName) {
    ...
    let data = {};
    try {
        const res = await (opts.fetch ?? defaultFetch)(query, {
            timeout: opts.timeout,
        });
        data = await res.json();
        return data;
...
async function getStartEnd(query, opts) {
    try {
        const startRes = await (opts.fetch ?? defaultFetch)(
            query + '%20ASC&resultRecordCount=1&f=pjson',
            {
                timeout: opts.timeout,
            }
        );
        const startId = await startRes.json();
        const endRes = await (opts.fetch ?? defaultFetch)(
            query + '%20DESC&resultRecordCount=1&f=pjson',
            {
                timeout: opts.timeout,
            }
        );
